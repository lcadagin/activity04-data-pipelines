...
**Response**: When the message argument is set to TRUE, any messages are
hidden when the file is knitted. When the message argument is set to
FALSE, any messages are shon when the file is knitted.
...
**Response**: The message option determines if a command’s message is
shown in a knitted file. The option is currently set to TRUE (or on), so
the message does not show in the knitted file.
...
**Response**: To answer this question, we need the variables major and
unemployment\_rate. If I were to use excel, I would sort the
unemployment\_rate field from lowest to highest. This would show me the
major with the lowet unemployment rate at the top.
...
**Response**: The major with the lowest unemployment rate from the
dataset is “Mathematics and Computer Science” (Rank 53).
...
**Response**: I needed to use the desc() function to arrange the
sharewomen from greatest to smallest. This shows me that Early Childhood
Education is the major with the highest proportion of women.
...
**Response**: We see from the output that there are 10 STEM majors that
earn less than the median of all majors (36000) including zoology,
ecology, and biology.
